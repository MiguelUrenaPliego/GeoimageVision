...
dataset_name = "ORTO_2023_10_90"
model_log_folder = base_dir + 'MadridDataset/UnetSAM/ExpoLoss/' + dataset_name
dataset_folder = base_dir + 'MadridDataset/res10cm_overlap10m'
img_dir = dataset_folder + "/" + dataset_name
mask_dir = dataset_folder + "/" + "GT"

train_folders = ['BERRUGUETE','COSTILLARES', 'EL_VISO','CASTELLANA','QUINTANA',
                 'EMBAJADORES','PUERTA_DEL_ANGEL','LOS_ROSALES','ACACIAS','GOYA',
                 'NUMANCIA','PALOMENAS__BAJAS','VALDERRIVAS',
                 'ORCASITAS','ALMENDRALES','UNIVERSIDAD','ALMAGRO','BELLAS_VISTAS',
                 'HISPANOAMERICA']
val_folders = ['GAZTAMBIDE', 'CUATRO_CAMINOS','PILAR','ARCOS','SAN_DIEGO']
...
dm = Seg.get_Madrid_train_datamodule(img_dir = img_dir, mask_dir = mask_dir, 
...
resolution = 0.1 # img resolution m
overlap = 10 # img overlap m

# ulabeled, building, street,       sidewalk, pool,       bike lane, parking
clean_mask_area = np.array([2,45,100,8  ,9,6  ,4]) # min area m^2
clean_mask_len =  np.array([0,6 ,1  ,0.5,2,0.5,1]) # min width m
...
for folder in val_folders: 
    test_dm = Seg.get_parking_test_datamodule(img_dir,test_folders=[folder])
